...
def display_same_replica_count_rg(
    ...
    print("=" * 40)
    for rg_id, replica_count in duplicate_replica_count_per_rg.items():
...
def display_same_topic_partition_count_broker(
    ...
    for broker_id, partition_count in \
            same_topic_partition_count.items():
        print(
...
def display_partition_count_per_broker(
    ...
    print("=" * 25)
    for broker_id, count in partition_count.items():
        print(
            "{b:^7d} {cnt:^18d}".format(
                b=broker_id,
                cnt=count,
...
def display_leader_count_per_broker(leader_count, stdev_imbalance, imbalance):
    ...
    print("=" * 33)
    for broker_id, count in leader_count.items():
        print(
            "{b:^7d} {cnt:^20d}".format(
                b=broker_id,
                cnt=count,
